...
example {p : ℝ≥0∞} [Fact (1 ≤ p)] (hp : p ≠ ⊤) (hp1 : 1 < p)
  ...
  sourceIsolatingSphere_subset_resolventSet hp hp1 φ ψ N ε hε hcluster hdisjoint n
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)]
    ...
    {z : ℂ} (hz : z ∈ sourceClusterDisc hp hp1 φ (fun _ => ε) i) :
    Real.pi/4 ≤ ‖sourceStandardRoot hp hp1 ψ j z‖ :=
  sourceStandardRoot_central_negative_tail_lower hp hp1 φ ψ hφ N ε
    hε hεmax houter hi hj hcluster hz
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)]
    ...
    {z : ℂ} (hz : z ∈ refinedResonantDisk j) :
    Real.pi/4 ≤ ‖sourceStandardRoot hp hp1 ψ i z‖ :=
  sourceStandardRoot_negative_tail_central_lower hp hp1 φ ψ hφ N ε
    hε hεmax houter hi hj hcluster hz
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)]
    ...
    (∫ᶜ z in γ, NLS.ComplexAnalysis.holomorphicOneForm
      (fun w => (sourceStandardRoot hp hp1 ψ n w)⁻¹) z) = 0 :=
  sourceStandardRoot_inv_curveIntegral_of_homotopy_circle_off_index_eq_zero
    hp hp1 φ ψ N ε m n c r hr hfilled hcluster hdisjoint
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)]
    ...
    (2*Real.pi*Complex.I)⁻¹ *
      (∫ᶜ z in γ, NLS.ComplexAnalysis.holomorphicOneForm
        (fun w => (sourceStandardRoot hp hp1 ψ n w)⁻¹) z) =
      if m = n then -1 else 0 :=
  normalized_sourceStandardRoot_inv_curveIntegral_indexed_of_homotopy_circle
    hp hp1 φ ψ N ε m n c r hr hsegm hfilled hcluster hdisjoint
...
example (τ δ : ℂ) (f : ℂ → ℂ) (t : ℝ)
    (htl : -1 ≤ t) (htr : t ≤ 1) :
    (∫ r in (-1:ℝ)..t,
      f (τ+δ*(r:ℂ)) / (Real.sqrt (1-r^2):ℂ)) =
    ∫ θ in Real.arccos t..Real.pi,
      f (τ+δ*(Real.cos θ:ℂ)) :=
  gap_weighted_integral_eq_cos τ δ f t htl htr

example (τ δ : ℂ) (f : ℂ → ℂ) (t : ℝ) (hδ : δ ≠ 0)
    (htl : -1 ≤ t) (htr : t ≤ 1) (upper : Bool) :
    gapSidePathIntegral τ δ f (-1) t upper =
      gapSideBoundaryIntegral τ δ f t upper :=
  gapSidePathIntegral_eq_boundary τ δ f t hδ htl htr upper

example (τ δ : ℂ) (f : ℂ → ℂ) (hδ : δ ≠ 0)
    (hf : ContinuousOn f (standardRootGapSegment τ δ)) (upper : Bool) :
    Tendsto (fun ε : ℝ =>
      gapSidePathIntegral τ δ f (-1+ε) (1-ε) upper)
      (𝓝[>] (0:ℝ))
      (𝓝 (gapSideBoundaryIntegral τ δ f 1 upper)) :=
  gapSidePathIntegral_double_trunc_tendsto τ δ f hδ hf upper
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)] (hp : p ≠ ⊤) (hp1 : 1 < p)
    (ψ : CoeffPair p) :
    IsOpen (sourceStandardRootPairedDomain hp hp1 ψ) :=
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)] (hp : p ≠ ⊤) (hp1 : 1 < p)
    ...
    HasProdLocallyUniformlyOn
      ...
      (sourceStandardRootPairedDomain hp hp1 ψ) :=
  hasProdLocallyUniformlyOn_sourceStandardRootPairedFactor hp hp1 ψ
...
/- Lemma 10.8: eventual half-unit rows and large-index cutoffs. -/
noncomputable section
open scoped ENNReal
...
example {p : ℝ≥0∞} [Fact (1 ≤ p)]
    ...
    ∃ K : ℕ, ∀ n : ℤ, K ≤ n.natAbs →
      (C^2/4) *
        (∑' m : ℤ, sourceSquaredGapReciprocalTerm hp hp1 ψ n m) ≤ 1/2 :=
  exists_sourceSquaredGapPhysicalRows_half_unit hp hp1 ψ C hC
...
example {p q : ℝ≥0∞} [Fact (1 ≤ p)] [Fact (1 ≤ q)]
  ...
  exists_sourceSingleRootQuotientPartialProduct_tail_bound
